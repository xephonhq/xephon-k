			...
			case <-tickChan:
				...
				if enableCPU {
					...
					for i, p := range cores {
						...
						s.Points = append(s.Points, common.IntPoint{TimeNano: currentTime, V: int64(stat.User)})
						s = seriesMap[p+"nice"]
						s.Points = append(s.Points, common.IntPoint{TimeNano: currentTime, V: int64(stat.Nice)})
						s = seriesMap[p+"system"]
						s.Points = append(s.Points, common.IntPoint{TimeNano: currentTime, V: int64(stat.System)})
						s = seriesMap[p+"idle"]
						s.Points = append(s.Points, common.IntPoint{TimeNano: currentTime, V: int64(stat.Idle)})
						s = seriesMap[p+"iowait"]
						s.Points = append(s.Points, common.IntPoint{TimeNano: currentTime, V: int64(stat.IOWait)})
						s = seriesMap[p+"irq"]
						s.Points = append(s.Points, common.IntPoint{TimeNano: currentTime, V: int64(stat.Irq)})
						s = seriesMap[p+"softirq"]
						s.Points = append(s.Points, common.IntPoint{TimeNano: currentTime, V: int64(stat.SoftIrq)})
						s = seriesMap[p+"steal"]
						s.Points = append(s.Points, common.IntPoint{TimeNano: currentTime, V: int64(stat.Steal)})
						s = seriesMap[p+"guest"]
						s.Points = append(s.Points, common.IntPoint{TimeNano: currentTime, V: int64(stat.Guest)})
						s = seriesMap[p+"guestnice"]
						s.Points = append(s.Points, common.IntPoint{TimeNano: currentTime, V: int64(stat.GuestNice)})
					}
				}
				s = seriesMap["mem.total"]
				s.Points = append(s.Points, common.IntPoint{TimeNano: currentTime, V: int64(memCollector.MemTotal)})
				s = seriesMap["mem.free"]
				s.Points = append(s.Points, common.IntPoint{TimeNano: currentTime, V: int64(memCollector.MemFree)})
